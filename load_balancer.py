...
def route_home_request_to_server():
    server = route_request()
    print("Selected Server:", server)  # Add this line for logging
    if server:
        try:
            response = requests.get(f"{server}/home")
            if response.status_code == 200:
                return response.text, 200  # Return the response content with status code 200
            else:
                return f"Error: {response.status_code}", response.status_code  # Return error status code
